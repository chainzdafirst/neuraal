...
export function PreviewAITutor() {
  return (
    <div className="bg-background rounded-[16px] border border-border overflow-hidden shadow-lg text-left">
      ...
      {/* Chat */}
      <div className="p-4 sm:p-5 md:p-6 space-y-3 text-[11px] sm:text-xs leading-relaxed">
        {/* User */}
        <div className="flex justify-end">
          <div className="bg-primary text-primary-foreground rounded-xl rounded-br-sm px-3 py-2 max-w-[80%]">
            Explain the law of diminishing marginal returns
          </div>
        </div>
        {/* Assistant */}
        <div className="flex justify-start">
          <div className="bg-card rounded-xl border border-border/50 p-3 sm:p-4 w-full space-y-2">
      ...
      {/* Input */}
      <div className="border-t border-border/50 px-4 py-2.5 flex items-center gap-2 bg-white/80">
        <div className="flex-1 h-8 rounded-md border border-border bg-background px-3 text-[11px] text-muted-foreground flex items-center">Ask Neuraal anything...</div>
        <div className="w-8 h-8 rounded-full bg-gradient-to-r from-primary to-accent flex items-center justify-center"><Send className="w-3.5 h-3.5 text-white" /></div>
      </div>
    </div>
  );
}
    ...
    <div className="bg-background rounded-[16px] border border-border overflow-hidden shadow-lg text-left">
      ...
      <div className="p-4 sm:p-5 md:p-6 space-y-3">
        ...
        <div className="space-y-2 text-[11px] sm:text-xs">
          ...
          map((o, i) =>
          <div key={i} className={`p-2.5 sm:p-3 rounded-lg border-2 flex items-center gap-2 ${o.correct ? "border-neuraal-emerald bg-neuraal-emerald/10" : "border-border opacity-50"}`}>
              <div className={`w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-medium ${o.correct ? "bg-neuraal-emerald text-white" : "bg-secondary"}`}>
        ...
        <div className="bg-accent/5 border border-accent/20 rounded-lg p-3 sm:p-4 text-[11px] sm:text-xs">
    ...
    <div className="bg-background rounded-[16px] border border-border overflow-hidden shadow-lg text-left">
      <div className="border-b border-border/50 px-4 h-10 flex items-center gap-2 bg-white/80">
        ...
        <div className="min-w-0">
          ...
          <span className="text-[9px] text-muted-foreground ml-2 hidden sm:inline">Project_Management.pdf</span>
        </div>
      </div>
      <div className="p-4 sm:p-5 md:p-6 text-[11px] sm:text-xs leading-relaxed space-y-3">
        <div>
  ...
  return (
    <div className="bg-background rounded-[16px] border border-border overflow-hidden shadow-lg text-left">
      ...
      <div className="p-4 sm:p-5 md:p-6">
        ...
        <div className="bg-gradient-to-br from-primary/5 to-accent/5 rounded-xl border border-border/50 p-5 sm:p-6 text-center min-h-[120px] flex flex-col items-center justify-center">
        ...
        <div className="mt-4 flex gap-2">
          <div className="flex-1 border border-destructive/30 rounded-lg py-2.5 flex items-center justify-center gap-1.5 text-[11px] sm:text-xs">
            <X className="w-3 h-3 text-destructive" /> Needs Practice
          </div>
          <div className="flex-1 border border-neuraal-emerald/30 rounded-lg py-2.5 flex items-center justify-center gap-1.5 text-[11px] sm:text-xs">
    ...
    <div className="bg-background rounded-[16px] border border-border overflow-hidden shadow-lg text-left">
      ...
      <div className="p-4 sm:p-5 md:p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          ...
          map((s) =>
          <div key={s.label} className="bg-card rounded-lg border border-border/50 p-2.5">
        ...
        {/* Chart */}
        <div className="bg-card rounded-lg border border-border/50 p-3 sm:p-4">
          <div className="text-[11px] sm:text-xs font-semibold mb-3 flex items-center gap-1.5">
          ...
          <div className="flex items-end justify-between h-24 gap-1.5">
            ...
            <div key={d.day} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[8px] sm:text-[9px] text-muted-foreground">{d.day}</span>
              </div>
            )}
          </div>
          <div className="mt-2 pt-2 border-t border-border flex justify-between text-[10px] sm:text-[11px]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          <div className="bg-card rounded-lg border border-border/50 p-3">
            <div className="text-[10px] sm:text-[11px] font-semibold mb-2 flex items-center gap-1"><AlertTriangle className="w-3 h-3 text-neuraal-amber" /> Improve</div>
            {[{ t: "Communication", s: 35 }, { t: "Microeconomics", s: 42 }].map((a) =>
            <div key={a.t} className="mb-1.5">
                <div className="flex justify-between text-[9px] sm:text-[10px] mb-0.5"><span>{a.t}</span><span className="text-neuraal-amber">{a.s}%</span></div>
                <div className="h-1.5 bg-secondary rounded-full"><div className="h-full bg-neuraal-amber rounded-full" style={{ width: `${a.s}%` }} /></div>
              </div>
            )}
          </div>
          <div className="bg-card rounded-lg border border-border/50 p-3">
            <div className="text-[10px] sm:text-[11px] font-semibold mb-2 flex items-center gap-1"><Zap className="w-3 h-3 text-neuraal-emerald" /> Strengths</div>
            {[{ t: "Agile Methods", s: 92 }, { t: "Project Mgmt", s: 85 }].map((a) =>
            <div key={a.t} className="mb-1.5">
                <div className="flex justify-between text-[9px] sm:text-[10px] mb-0.5"><span>{a.t}</span><span className="text-neuraal-emerald">{a.s}%</span></div>
                <div className="h-1.5 bg-secondary rounded-full"><div className="h-full bg-neuraal-emerald rounded-full" style={{ width: `${a.s}%` }} /></div>
              </div>
            )}
          </div>
        </div>
